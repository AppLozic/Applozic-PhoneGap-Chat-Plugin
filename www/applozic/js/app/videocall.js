...
function MckCallingService(identity,token,callId,toUserDisplayName,isCallHost,callStartTime,mckMessageService,toUserImage,isAudioCall) {
    var _this=this;
    ...
    var ringToneUrl =  "/applozic/audio/applozic_video_call_ring_tone.mp3";
    // load videoCall ringTone
    _this.ringTone =  new RingToneService().loadRingTone(ringToneUrl);
    ...
    _this.startVideoCall = function(){
        ...
        var twilioService = new TwilioService(_this.identity, _this.token, _this.callId, _this.ringTone, _this.isAudioCall,_this.mckMessageService);
        var videoClient = twilioService.InitializeVideoClient();

        if (isCallHost) {
            $mckVidImageIcon.html(toUserImage + "<span> Calling " + toUserDisplayName + "</span>");
            ...
            setTimeout(function() {
                ...
                if (!twilioService.isCallReceived) {
                    console.log("call is not answered....");
                    _this.ringTone.stop();
                    mckMessageService.sendVideoCallMessage(callId, "CALL_MISSED", 103, isAudioCall);
                    alert(toUserDisplayName + " not Available....");
                    twilioService.leaveRoomIfJoined();
                    //window.close();
        ...
        twilioService.attachUserMedia(isCallHost, isAudioCall);
        // join Call,connect user to room
        twilioService.joinCall(isAudioCall);
        ...
        $applozic("#mck-vid-disconnect").off('click').on('click', function() {
            ...
            if (isCallHost) {
                if (twilioService.callReceivedAt) {
                    // call received. send call end message.
                    var callDurationInMilis = new Date().getTime() - twilioService.callReceivedAt.getTime();
                    mckMessageService.sendVideoCallEndMessage(callId, "CALL_END", 103, false, callDurationInMilis);
                } else {
                    // call is not received. send call missed message.
                    mckMessageService.sendVideoCallMessage(callId, "CALL_MISSED", 103, false);
                }
            }
            // no need to handle receiver's call disconnects message. sender
            // will a notification when participent disconnects. sender will
        ...
        $mckSideBox.addClass('n-vis').removeClass('vis');
        $mck_vid_box.removeClass('n-vis').addClass('vis');
    };
 };


function TwilioService(identity, token, callId, ringTone, isAudioCall,mckMessageService) {
   var _this = this;
    _this.identity = identity;
    _this.token = token;
    _this.callId = callId;
    ...
    _this.roomJoined = function(room) {
        ...
        // callback when participent join the call. logic what happen on host
        // browser should be here
        room.on('participantConnected', function(participant) {
            console.log("participent- " + participant.identity + "connected to the room- " + room);
            // stop the ringtone
        ...
        // When a participant disconnects, it will execute on  senders browser.
        room.on('participantDisconnected', function(participant) {
            console.log("Participant '" + participant.identity + "' left the room");
            var callDurationInMilis = new Date().getTime() - _this.callReceivedAt.getTime();
            mckMessageService.sendVideoCallEndMessage( _this.callId, "CALL_END", 103, isAudioCall, callDurationInMilis)
            participant.media.detach();
            //right now one to one calling.. if participent disconnect disconnect the call.
            //window.close();
            $mck_vid_box.addClass('n-vis').removeClass('n-vis');
        });
